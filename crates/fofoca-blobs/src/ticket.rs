...
use fofoca_protocol::peer_addr::{endpoint_addr_from_json, endpoint_addr_to_json};
...
mod tests {
    ...
    /// Blob tickets and invite tickets share the bare-base58 shape and differ
    /// only by kind byte, so each must reject the other. Lives here rather
    /// than beside `InviteTicket`: `fofoca-protocol` is a layer below
    /// and cannot see `BlobTicket`.
    #[test]
    fn an_invite_and_a_blob_ticket_never_cross_parse() {
        use fofoca_protocol::mesh::{Mesh, MeshConfig, MeshName};
        use fofoca_protocol::{InviteTicket, mesh::LookupOpts};
        use iroh::SecretKey;
        ...
        .encode();
        let invite = fofoca::ops::invite::mint(&creator, Some(3600), None).unwrap();
        assert!(InviteTicket::decode(&blob).is_err());
        assert!(BlobTicket::decode(&invite).is_err());
    }
    use crate::{HASH_LEN, SECRET_LEN};
    use fofoca_protocol::mesh::LookupOpts;
    ...
    fn rejects_a_mesh_token() {
        ...
        let mesh = fofoca_protocol::mesh::Mesh::new(
            [1u8; 32],
            fofoca_protocol::mesh::MeshName::new("t").unwrap(),
            fofoca_protocol::mesh::MeshConfig::loopback(),
        )
        .to_string();
